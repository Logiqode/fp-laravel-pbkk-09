<x-layout>
    ...
    @elseif(session()->has('addListingSuccess'))
        <div id="toast-success"
            class="flex items-center w-full p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400 dark:bg-gray-800"
            role="alert">
            <div
            ...
            <div class="ms-3 text-sm font-normal">{{ session('addListingSuccess') }}</div>
            <button type="button"
                class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700"
                data-dismiss-target="#toast-success" aria-label="Close">
                <span class="sr-only">Close</span>
    ...
    @if ($storeOwner->status == 'Pending')
        @if ($storeOwner->user_id == Auth::id())
            <h1>Thank you for requesting a store. Your store application is currently under review of an Admin. We'll
                send you a notification once your store is Verified</h1>
        @else
            <h1>This store is not open for business yet.</h1>
        @endif
    @elseif ($storeOwner->status == 'Suspended')
        @if ($storeOwner->user_id == Auth::id())
            <h1>Your store has been suspended. Please contact an Admin to make an appeal.</h1>
        @else
            <h1>This store is currently suspended.</h1>
        @endif
    @elseif ($storeOwner->status == 'Verified')
        <section class="py-8 antialiased md:py-16">
            <div class="mx-auto grid max-w-screen-xl px-4 pb-8 md:grid-cols-12 lg:gap-12 lg:pb-16 xl:gap-0">
                <div class="content-center justify-self-start md:col-span-7 md:text-start">
                    ...
                    <a href="#lists"
                        ...
                        Now</a>
                    @if ($storeOwner->user_id == Auth::id())
                        <a class="py-4 mx-2 px-6 w-full text-base font-medium text-center text-white rounded-lg border cursor-pointer bg-green-700 border-green-600 hover:bg-green-800 focus:ring-4 focus:ring-green-300 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800"
                            href="/store/add">Add Listing</a>
                    @endif
                </div>
            ...
            <div
                class="mx-auto grid max-w-screen-xl grid-cols-2 py-4 gap-8 text-gray-500 dark:text-gray-400 sm:grid-cols-3 sm:gap-12 lg:grid-cols-6 px-4">
                <a class="flex items-center md:justify-center">
